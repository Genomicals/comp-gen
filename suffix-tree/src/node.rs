...
impl Node {
    ...
    pub fn find_path_new(root: Rc<RefCell<Node>>, string: &str) {
        let mut current_str: String;
        let mut target_str: String;

    }

    pub fn find_path(rc: Rc<RefCell<Node>>, string: &str, index: usize) {
        ...
        if target_str.contains(&current_str) { //if the target fully contains the current string
            ...
            for child in &rc_children {
                if child.borrow().get_string(&string).as_bytes()[0] == rest_str.as_bytes()[0] { //found the next child
                    ...
                    Node::find_path(child.clone(), string, index + cur_len); //recursion
                    return;
                } 
            }
            ...
            rc.borrow_mut().children.push(Rc::new(RefCell::new(new_node)));
